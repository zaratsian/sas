...
url          = 'http://localhost:41001/SASESP/windows//Sensors/CQ1/Data_Stream/state?value=injected'
...
while True:
    ...
    requests.put(url, data=str('''
        <events>
            <event>
                <opcode>i</opcode>
                <id key='true'>'''  + str(datetime.datetime.now().strftime(dateformat))     + '''</id>
                <location>pi106</location>
                <timestamp>'''      + str(datetime.datetime.now().strftime(dateformat))     + '''</timestamp>  
