 ...
 /*                                                             		*/
 /* CREATORS: Vinicius Vivaldi, Dan Zaratsian							*/
...
%let CC_project = saschats_products;

/* Data source to be scored */
%let input_data = demodata.jobs;

/* Output location of the final scored dataset*/
%let output_data = demodata.jobs_cat;
...
data _null_;
	...
	cmd2 = "python CatConClientTextMiner.py --server &CC_Server_name.:&CC_Server_port. --show-paths --dir &dataloc. --show-relevancy --categorize --categorization-project &CC_project. > &scoreloc.";
...
data &output_data (compress=yes);
    ...
    informat category $250.;
    informat relevancy 8.2;
    format filename $250.;
    format category $250.;
    format relevancy 8.2;
    input
    filename $ 
    category $
    relevancy
...
data &output_data; set &output_data; 
	category_level_1 = scan(category,2,'/');
	category_level_2 = scan(category,3,'/');
	category_level_3 = scan(category,4,'/');
	category_level_4 = scan(category,5,'/');
	category_level_5 = scan(category,6,'/');
	drop category filename;
	run;
...
data &output_data (compress=YES); set &output_data; 
	if category_level_1 = '' then category_level_1 = 'Other';
	if category_level_2 = '' then category_level_2 = 'Other';
	if category_level_3 = '' then category_level_3 = 'Other';
	if category_level_4 = '' then category_level_4 = 'Other';
	if category_level_5 = '' then category_level_5 = 'Other';
	if relevancy = . then relevancy = 0;
	run;
