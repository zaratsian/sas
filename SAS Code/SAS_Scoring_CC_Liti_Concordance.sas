...
libname z 'C:\Users\dazara\Desktop';
...
%let python_location = C:\Program Files\Teragram\Teragram Catcon Server\client_api\python;
...
%let CC_project = linkedin;

/* Data source to be scored */
%let input_data = z.linkedin_combinedasof_07_30_14_n;

/* Output location of the final scored dataset*/
%let output_data = z.linkedin_scored;
...
data _null_;
	...
	cmd2 = "python CatConClientTextMiner_Concordance.py --server &CC_Server_name.:&CC_Server_port. --show-paths --dir &dataloc. --extract-li-facts --liti-project &CC_project. --output_dir &scoreloc.";
...
data &output_data;
	infile scored delimiter = '|' MISSOVER DSD lrecl=32767 firstobs=2 ;
	informat filename $200.;
	informat concept  $200.;
	informat concept_match $200.;
	informat concordance $1000.;
	format filename $200.;
	format concept  $200.;
	format concept_match $200.;
	format concordance $1000.;
input
	filename $
	concept  $
	concept_match $
...
set &output_data;
INTERNAL_ID_ZQJ = input(scan(filename,1,"."), best12.);
run;
