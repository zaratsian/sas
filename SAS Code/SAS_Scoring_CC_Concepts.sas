...
libname demodata 'C:\Users\dazara\Desktop\SAS - Projects\Data';
...
%let CC_project = concepts_project;

/* Data source to be scored */
%let input_data = demodata.rawdata;

/* Output location of the final scored dataset*/
%let output_data = work.scored_data;
...
data _null_;
	...
	cmd2 = "python CatConClientTextMiner.py --server &CC_Server_name.:&CC_Server_port. --show-paths --dir &dataloc. --concept-project &CC_project. --extract-concepts > &scoreloc.";
...
data &output_data (compress=yes);
    ...
    informat concept $250.;
    informat concept_match $250.;
    format filename $250.;
    format concept $250.;
    format concept_match $250.;
    input
    filename $ 
    concept $
    concept_match $
...
data demodata.jnj_concepts_20130809 (encoding='UTF-8'); set scored_data; run;
